# Atlas — Specification v0.3.2

**Status:** révision normative ciblée de la v0.3.1 après confrontation aux premières règles structurées persistées ; consolidation des POC Semantic Kernel, Candidate Discovery et Temporal Planning  
...
Le vocabulaire relationnel DOIT être extensible. La v0.3.2 retient comme formes déjà démontrées :
...
La v0.3.2 ne prétend pas que la découverte automatique de toute décomposition ou convergence sémantique est résolue.
...
La v0.3.2 ne requiert pas une décompilation sémantique générale.
...
La v0.3.2 n'impose pas de spécialisation dynamique à l'exécution. Elle exige seulement que le modèle ne l'interdise pas conceptuellement.
...
La v0.3.2 ne définit pas une politique multiobjectif universelle.
...
Le grounding DOIT exposer un statut de couverture. La v0.3.2 retient au minimum les classes conceptuelles suivantes :
...
La v0.3.2 n'impose pas une théorie universelle de décision sous incertitude.
...
La v0.3.2 distingue conceptuellement les responsabilités suivantes sans imposer qu'elles correspondent à des processus ou modules séparés.
...
- **R1 — abstraction sémantique fondamentale : fortement dérisqué.**
  `Description + relations/faits` constitue la base v0.3.2.
...
La v0.3.2 impose qu'une sémantique d'incertitude soit déclarée, mais ne choisit pas de théorie universelle. L'intégration de coûts incertains, faits estimés, contraintes temporelles et décisions jointes reste partiellement ouverte, notamment pour les politiques robustes ou scénarisées à grande échelle.
...
La sélection abstraite est mieux comprise que la production automatique d'artefacts fidèles et leur validation. La v0.3.2 introduit des obligations explicites entre solution, materializer, toolchain et validator ; leur couverture pratique reste à démontrer sur une cible réelle.
...
Le premier travail sur les règles structurées montre en outre qu'une représentation trop pauvre peut perdre des participants, de l'ordre ou des préconditions sémantiquement pertinents. La v0.3.2 impose leur préservation lorsque nécessaire, sans imposer une AST universelle.
...
Une implémentation se réclamant du noyau Atlas v0.3.2 DOIT au minimum respecter les invariants suivants :
...
4. les règles d'identité, d'égalité, d'ordre, de hachage et de coercition qui affectent la sémantique sont définies par Atlas et non introduites implicitement par le langage hôte ;
5. un prédicat utilisé pour le raisonnement possède une sémantique stable ;
6. une propriété utilisée par une règle est résolue relativement à l'identité du participant auquel elle s'applique ;
7. une conclusion relationnelle dérivée conserve son prédicat, ses participants groundés et ses dépendances effectives de manière récupérable ;
8. un résultat booléen d'évaluation n'est pas assimilé à lui seul à une conclusion relationnelle groundée ;
9. une relation n'est fonctionnalisée que si l'unicité pertinente est déclarée, dérivée ou vérifiée ;
10. toute structure interne, tout ordre et toute précondition qui influencent la validité, l'applicabilité ou les conséquences restent exploitables par le raisonnement ;
11. l'absence d'une précondition requise n'est pas interprétée comme sa satisfaction ;
12. possibilité, constructibilité, admissibilité et sélection sont distinctes ;
13. les décisions sont contextualisées par workload, contraintes et plateforme lorsque pertinent ;
14. les coûts de préparation et de partage peuvent être comptés à l'échelle du plan, pas seulement localement par appel ;
15. le passage du corpus en monde ouvert vers un problème fini est explicité par un `Decision Scope`, une `Grounding Policy` et un statut de grounding ;
16. un résultat `optimal` est toujours qualifié relativement au `Grounded Decision Problem` résolu ;
17. le système n'exige pas l'énumération préalable de toutes les solutions complètes ;
18. le modèle peut représenter conjointement choix et temps lorsqu'ils interagissent ;
19. le peak de ressources peut dépendre des lifetimes plutôt que d'une somme statique ;
20. une connaissance utilisée pour une décision peut être reliée à sa provenance et à son domaine de validité ;
21. toute dérivation persistée conserve des dépendances structurées suffisantes pour son audit et sa révision ;
22. les frontières d'écriture et de lecture du Knowledge Store font respecter des invariants sémantiques équivalents ;
23. la validité syntaxique d'un identifiant de propriété n'implique pas que cette propriété appartient au vocabulaire consommé par une règle ;
24. une décision utilisant des valeurs non exactes déclare la sémantique selon laquelle l'incertitude est interprétée ;
25. la matérialisation est distincte de la connaissance de plateforme et de la décision sémantique ;
26. les hypothèses dont dépend une matérialisation sont conservées sous forme d'obligations garanties, encodées, vérifiées ou explicitement rejetées ;
27. une solution ou un échec est qualifié relativement au corpus, au contexte et au grounding courants ;
28. le corpus peut être enrichi ou corrigé sans traiter les matérialisations précédentes comme source de vérité unique ;
29. la capacité limitée d'un évaluateur courant ne ferme pas normativement le vocabulaire du corpus persistant.
...
La hiérarchie de valeur retenue par la v0.3.2 est :
