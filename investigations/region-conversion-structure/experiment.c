...
static void one(const char *name, Shape shape) {
    ...
    QROB0 ba={0},bf={0},result={0}; QROB1 b1={0}; QROStats st={0};
    if(qro_b0_build(&a,&ba,NULL)||qro_b0_build(&full,&bf,NULL)||qro_b0_op(&ba,&bf,QRO_INTERSECT,&result,NULL))exit(3);
    QROMask view={result.width,result.height,result.stride,result.bits}; if(qro_b1_build(&view,&b1,&st))exit(4);
    uint64_t c[SAMPLES],r0[SAMPLES],r1[SAMPLES];
    ...
    for(int i=0;i<SAMPLES;i++){uint64_t t=now_ns();for(int z=0;z<BATCH;z++)qro_b0_apply(&result,&src,rr,&dst,rr);r0[i]=(now_ns()-t)/BATCH;t=now_ns();for(int z=0;z<BATCH;z++)qro_b1_apply(&b1,&src,rr,&dst,rr);r1[i]=(now_ns()-t)/BATCH;}
    qsort(c,SAMPLES,sizeof*c,cmp_u64);qsort(r0,SAMPLES,sizeof*r0,cmp_u64);qsort(r1,SAMPLES,sizeof*r1,cmp_u64);
    ...
    printf("{\"shape\":\"%s\",\"area\":%d,\"density\":0.5,\"input_runs\":%" PRIu64 ",\"converted_storage_bytes\":%zu,\"conversion_median_ns\":%" PRIu64 ",\"apply_b0_ns\":%" PRIu64 ",\"apply_b1_ns\":%" PRIu64 ",\"point_estimate_N\":%" PRIu64 ",\"b0_canonical_hash\":\"%016" PRIx64 "\",\"b1_canonical_hash\":\"%016" PRIx64 "\",\"logical_identity\":%s}\n",name,W*H/2,run_count_mask(&a),st.storage_bytes,c[SAMPLES/2],r0[SAMPLES/2],r1[SAMPLES/2],threshold,h0,h1,h0==h1?"true":"false");
    free(srcd);free(dstd);qro_b1_free(&b1);qro_b0_free(&ba);qro_b0_free(&bf);qro_b0_free(&result);mask_free(&a);mask_free(&full);
}
