# Atlas — Specification v0.3.1

**Status:** révision normative ciblée de la v0.3.1 après revue critique ; consolidation des POC Semantic Kernel, Candidate Discovery et Temporal Planning  
...
Le vocabulaire relationnel DOIT être extensible. La v0.3.1 retient comme formes déjà démontrées :
...
La v0.3.1 ne prétend pas que la découverte automatique de toute décomposition ou convergence sémantique est résolue.
...
La v0.3.1 ne requiert pas une décompilation sémantique générale.
...
La représentation DOIT préserver suffisamment d'identité et de structure pour permettre le partage, la provenance et l'explication.
...
La v0.3.1 n'impose pas de spécialisation dynamique à l'exécution. Elle exige seulement que le modèle ne l'interdise pas conceptuellement.
...
La v0.3.1 ne définit pas une politique multiobjectif universelle.
...
Le grounding DOIT exposer un statut de couverture. La v0.3.1 retient au minimum les classes conceptuelles suivantes :
...
La v0.3.1 n'impose pas une théorie universelle de décision sous incertitude.
...
La v0.3.1 distingue conceptuellement les responsabilités suivantes sans imposer qu'elles correspondent à des processus ou modules séparés.
...
- descriptions génériques à identité nominale stable ;
...
- **R1 — abstraction sémantique fondamentale : fortement dérisqué.**
  `Description + relations/faits` constitue la base v0.3.1.
...
La v0.3.1 impose qu'une sémantique d'incertitude soit déclarée, mais ne choisit pas de théorie universelle. L'intégration de coûts incertains, faits estimés, contraintes temporelles et décisions jointes reste partiellement ouverte, notamment pour les politiques robustes ou scénarisées à grande échelle.
...
La sélection abstraite est mieux comprise que la production automatique d'artefacts fidèles et leur validation. La v0.3.1 introduit des obligations explicites entre solution, materializer, toolchain et validator ; leur couverture pratique reste à démontrer sur une cible réelle.

### R12 — coût opérationnel de provenance et d'explication

Aucun problème majeur n'est encore démontré, mais la collecte à grande échelle devra mesurer le coût de maintien de provenance, versions et domaines de validité.
...
Une implémentation se réclamant du noyau Atlas v0.3.1 DOIT au minimum respecter les invariants suivants :
...
4. possibilité, constructibilité, admissibilité et sélection sont distinctes ;
5. les décisions sont contextualisées par workload, contraintes et plateforme lorsque pertinent ;
6. les coûts de préparation et de partage peuvent être comptés à l'échelle du plan, pas seulement localement par appel ;
7. le passage du corpus en monde ouvert vers un problème fini est explicité par un `Decision Scope`, une `Grounding Policy` et un statut de grounding ;
8. un résultat `optimal` est toujours qualifié relativement au `Grounded Decision Problem` résolu ;
9. le système n'exige pas l'énumération préalable de toutes les solutions complètes ;
10. le modèle peut représenter conjointement choix et temps lorsqu'ils interagissent ;
11. le peak de ressources peut dépendre des lifetimes plutôt que d'une somme statique ;
12. une connaissance utilisée pour une décision peut être reliée à sa provenance et à son domaine de validité ;
13. une décision utilisant des valeurs non exactes déclare la sémantique selon laquelle l'incertitude est interprétée ;
14. la matérialisation est distincte de la connaissance de plateforme et de la décision sémantique ;
15. les hypothèses dont dépend une matérialisation sont conservées sous forme d'obligations garanties, encodées, vérifiées ou explicitement rejetées ;
16. une solution ou un échec est qualifié relativement au corpus, au contexte et au grounding courants ;
17. le corpus peut être enrichi ou corrigé sans traiter les matérialisations précédentes comme source de vérité unique.
...
La hiérarchie de valeur retenue par la v0.3.1 est :
